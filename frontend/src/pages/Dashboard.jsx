...
function Dashboard() {
  ...
  const COLORS = ["#9dff00", "#00c853", "#4caf50", "#8bc34a", "#009688"];
  ...
  const loadDashboard = async () => {
    try {
      const [
        ...
        API.get("/analytics/region-sales"),
        API.get("/analytics/revenue-prediction"),
        API.get("/analytics/inventory-risk"),
      ]);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10 mt-8">
          ...
          {regionSales.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={350}>
              <BarChart data={regionSales}>
                ...
                <YAxis width={90} />
                <Tooltip />
                <Bar dataKey="total_sales" fill="#00c853" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartBox>
